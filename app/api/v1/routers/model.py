...
from app.services.constant import DanceName, BACKGROUND_DIR, CHARACTER_DIR, RESULT_DIR, SOURCE_DIR, MODEL_RESULT_DIR
...
async def handle_model_request(x_cd_user_id: str = Header(...)):
    ...
    try:
        BlobService.get_result_image(save_path=f"{SOURCE_DIR}/{BACKGROUND_DIR}/{user_uuid}.png", blob_name=f"{BACKGROUND_DIR}/{user_uuid}.png")
        BlobService.get_result_image(save_path=f"{SOURCE_DIR}/{CHARACTER_DIR}/{user_uuid}.png", blob_name=f"{CHARACTER_DIR}/{user_uuid}.png")
